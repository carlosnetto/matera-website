  ...
  useEffect(() => {
    fetch('/data/en/blog/index.json')
      ...
      .then((index: BlogEntry[]) => {
        ...
        if (entry) {
          return fetch(`/data/en/blog/${encodeURIComponent(entry.file)}`).then(r => r.json());
        }
        return null;
      })
      .then(data => {
  ...
  if (!post) return (
    <div style={{ minHeight: '60vh', paddingTop: '8rem', textAlign: 'center' }}>
      <div className="container">
        ...
        <Link to="/en/blog" style={{ color: 'var(--matera-purple)', fontWeight: 600 }}>Back to Blog</Link>
      </div>
    </div>
  );
    ...
    <>
      ...
      <section style={{
        ...
        <div className="container" style={{ maxWidth: '800px' }}>
          <Link to="/en/blog" style={{ color: 'var(--matera-green)', fontSize: '0.9rem', fontWeight: 600, marginBottom: '24px', display: 'inline-block' }}>
            ← Back to Blog
          </Link>
...
export default function Blog() {
  return null; // Routed via App.tsx
}
